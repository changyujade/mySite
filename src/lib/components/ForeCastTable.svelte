<script>
  ...
  function formatDayLabel(/** @type {string} */ dateString) {
  ...
  function isRainyDay(/** @type {number} */ index) {
  ...
  <Table.Root>
    ...
    <Table.Body>
      {#each forecast.time as day, i}
        <Table.Row class={isRainyDay(i) ? "bg-black/10 border-l-4 border-l-black" : ""}>
          ...
          <Table.Cell class={isRainyDay(i) ? "font-semibold text-black" : ""}>
